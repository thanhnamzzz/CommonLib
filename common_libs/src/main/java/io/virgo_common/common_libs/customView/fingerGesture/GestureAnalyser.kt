...
class GestureAnalyser @JvmOverloads constructor(
    ...
    private fun initialFingerDist(fingerNum1: Int, fingerNum2: Int): Double {
        return sqrt((initialX[fingerNum1] - initialX[fingerNum2]).pow(2.0) + (initialY[fingerNum1] - initialY[fingerNum2]).pow(2.0))
    }

    private fun finalFingerDist(fingerNum1: Int, fingerNum2: Int): Double {
        return sqrt((finalX[fingerNum1] - finalX[fingerNum2]).pow(2.0) + (finalY[fingerNum1] - finalY[fingerNum2]).pow(2.0))
    }
